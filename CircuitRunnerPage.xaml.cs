...
public partial class CircuitRunnerPage : ContentPage, IQueryAttributable
{
    ...
    {
        pendingRoomName = ReadQueryValue(query, "room");
        pendingWallId = ReadQueryValue(query, "wallId");
        pendingCircuitId = ReadQueryValue(query, "circuitId");
        pendingAutoStart = string.Equals(ReadQueryValue(query, "autoStart"), "1", StringComparison.Ordinal);
    }
    ...
    protected override async void OnAppearing()
    {
        ...
        try
        {
            ...
            if (pendingAutoStart && selectedCircuit is not null)
            {
                pendingAutoStart = false;
                await RunVisualizeAndStartAsync();
            }
        }
        catch (Exception ex)
        {
    ...
    private void RefreshView()
    {
        ...
        SelectedCircuitLabel.Text = selectedCircuit is null
            ...
            : $"Circuito selezionato: {selectedCircuit.Name} - Pareti {selectedCircuit.WallSummary} - Diff {selectedCircuit.Difficulty} - Incl. {selectedCircuit.Inclination}";

        RebuildCircuitsList();
        CircuitsEmptyLabel.IsVisible = GetVisibleCircuits().Count == 0;
        var isHubCompatible = selectedCircuit is not null && selectedCircuit.GetWallNames().Count == 1;
        HubCompatibilityLabel.IsVisible = selectedCircuit is not null && !isHubCompatible;
        VisualizeCircuitButton.IsEnabled = isHubCompatible && !isBusyWithEsp32;
        StartCircuitButton.IsEnabled = isHubCompatible && !isBusyWithEsp32;
    ...
    private void ApplyPendingSelection()
    {
        ...
        {
            selectedRoomName = Uri.UnescapeDataString(pendingRoomName);
        }

        if (int.TryParse(Uri.UnescapeDataString(pendingWallId ?? string.Empty), out var wallId))
        {
            selectedWall = availableWalls.FirstOrDefault(wall => wall.Id == wallId);
            if (selectedWall is not null)
            {
                selectedRoomName = selectedWall.RoomName;
            }
        ...
        if (int.TryParse(Uri.UnescapeDataString(pendingCircuitId ?? string.Empty), out var circuitId))
        {
            selectedCircuit = availableCircuits.FirstOrDefault(circuit => circuit.Id == circuitId);
            if (selectedCircuit is not null)
            {
                selectedRoomName = selectedCircuit.RoomName;
                selectedWall = availableWalls.FirstOrDefault(wall =>
                    string.Equals(wall.RoomName, selectedCircuit.RoomName, StringComparison.Ordinal) &&
                    selectedCircuit.UsesWall(wall.Name));
            }
        }

        pendingRoomName = null;
        pendingWallId = null;
        pendingCircuitId = null;
    }
        ...
        foreach (var circuit in GetVisibleCircuits())
        {
            ...
            border.Content = new VerticalStackLayout
            {
                ...
                Children =
                {
                    ...
                    new Label
                    {
                        Text = $"Pareti: {circuit.WallSummary} - Movimenti: {circuit.DynamicMovementCount} - Piedi: {circuit.FootHoldCount}",
                        FontSize = 12,
    ...
    private async Task RunCircuitCommandAsync(string busyMessage, Func<Esp32DeviceSettings, Task<string>> action)
    {
        ...
        using var busy = AppBusy.Show(busyMessage);
        try
        {
    ...
    private async Task RunVisualizeAndStartAsync()
    {
        await RunCircuitCommandAsync(
            "Visualizzazione e avvio circuito in corso...",
            async settings =>
            {
                var circuitId = await EnsureSelectedCircuitSyncedAsync(settings);
                var visualizeResponse = await app.Esp32ApiClient.VisualizeCircuitAsync(settings, circuitId);
                if (!visualizeResponse.Success)
                {
                    return $"Visualizza KO - {visualizeResponse.ErrorCode} - {visualizeResponse.Message}";
                }

                var startResponse = await app.Esp32ApiClient.StartCircuitAsync(settings, circuitId);
                return startResponse.Success
                    ? $"Visualizza + Avvio OK - {selectedCircuit!.Name}"
                    : $"Avvio KO - {startResponse.ErrorCode} - {startResponse.Message}";
            });
    }
    ...
    private async Task<string> EnsureSelectedCircuitSyncedAsync(Esp32DeviceSettings settings)
    {
        ...
        if (selectedCircuit.GetWallNames().Count > 1)
        {
            throw new InvalidOperationException(
                "Il circuito usa piu pareti. Il protocollo RouteLab Hub attuale supporta un solo WallId per esecuzione; visualizzazione e avvio hardware saranno disponibili dopo l'estensione del firmware.");
        }
        ...
        {
            throw new InvalidOperationException($"Stato RouteLab Hub non disponibile: {statusResponse.ErrorCode} - {statusResponse.Message}");
        }
