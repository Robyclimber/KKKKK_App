...
{
    private enum RestColorTarget
    {
        Blink,
        Completed
    }

    private enum ResistanceColorTarget
    {
        Active,
        Completed
    }

    private enum HangColorTarget
    {
        Active,
        Completed
    }
    ...
    private bool isUpdatingRestColorControls;
    private bool isUpdatingResistanceColorControls;
    private bool isUpdatingHangColorControls;
    private RestColorTarget activeRestColorTarget = RestColorTarget.Blink;
    private ResistanceColorTarget activeResistanceColorTarget = ResistanceColorTarget.Active;
    private HangColorTarget activeHangColorTarget = HangColorTarget.Active;
    ...
    private void EnsureDefaultsLoaded()
    {
        ...
        BlinkColorValueLabel.Text ??= "#FF0000";
        CompletedColorValueLabel.Text ??= "#00FF00";
        RefreshRestColorPreviews();
        SetActiveRestColorTarget(activeRestColorTarget);
        ...
        ResistanceActiveColorValueLabel.Text ??= "#FF8C00";
        ResistanceCompletedColorValueLabel.Text ??= "#00FF00";
        RefreshResistanceColorPreviews();
        SetActiveResistanceColorTarget(activeResistanceColorTarget);

        HangDurationSecondsEntry.Text ??= "10";
        HangCompletedHoldSecondsEntry.Text ??= "3";
        HangActiveColorValueLabel.Text ??= "#00BFFF";
        HangCompletedColorValueLabel.Text ??= "#00FF00";
        RefreshHangColorPreviews();
        SetActiveHangColorTarget(activeHangColorTarget);
    ...
    private void OnPickBlinkColorClicked(object? sender, EventArgs e)
    {
        SetActiveRestColorTarget(RestColorTarget.Blink);
    }

    private void OnPickCompletedColorClicked(object? sender, EventArgs e)
    {
        SetActiveRestColorTarget(RestColorTarget.Completed);
    }

    private void OnPickResistanceActiveColorClicked(object? sender, EventArgs e)
    {
        SetActiveResistanceColorTarget(ResistanceColorTarget.Active);
    }

    private void OnPickResistanceCompletedColorClicked(object? sender, EventArgs e)
    {
        SetActiveResistanceColorTarget(ResistanceColorTarget.Completed);
    }

    private void OnPickHangActiveColorClicked(object? sender, EventArgs e)
    {
        SetActiveHangColorTarget(HangColorTarget.Active);
    }

    private void OnPickHangCompletedColorClicked(object? sender, EventArgs e)
    {
        SetActiveHangColorTarget(HangColorTarget.Completed);
    }
    ...
    {
        if (isUpdatingRestColorControls)
        {
            return;
        }
        ...
        ApplyRestColorToTarget(activeRestColorTarget, hex);
        UpdateRestColorPickerTexts(color);
    }

    private void OnResistanceColorSliderChanged(object? sender, ValueChangedEventArgs e)
    {
        if (isUpdatingResistanceColorControls)
        {
            return;
        }

        var color = Color.FromRgb((byte)ResistanceRedSlider.Value, (byte)ResistanceGreenSlider.Value, (byte)ResistanceBlueSlider.Value);
        var hex = ToHexColor(color);
        ApplyResistanceColorToTarget(activeResistanceColorTarget, hex);
        UpdateResistanceColorPickerTexts(color);
    }

    private void OnHangColorSliderChanged(object? sender, ValueChangedEventArgs e)
    {
        if (isUpdatingHangColorControls)
        {
            return;
        }

        var color = Color.FromRgb((byte)HangRedSlider.Value, (byte)HangGreenSlider.Value, (byte)HangBlueSlider.Value);
        var hex = ToHexColor(color);
        ApplyHangColorToTarget(activeHangColorTarget, hex);
        UpdateHangColorPickerTexts(color);
    }
        ...
        foreach (var workout in visibleWorkouts)
        {
            ...
            header.Add(new VerticalStackLayout
            {
                ...
                Children =
                {
                    ...
                    new Label
                    {
                        Text = $"{workout.Steps.Count} step · {BuildWorkoutDurationSummary(workout)}",
                        FontSize = 12,
                        TextColor = Color.FromArgb("#D8A72D")
                    }
                }
            ...
            foreach (var phase in plan)
            {
                ...
                phaseHeader.Add(new VerticalStackLayout
                {
                    ...
                    Children =
                    {
                        ...
                        new Label
                        {
                            Text = $"{GetStepTypeLabel(phase.StepType)} · rip {phase.Repetition}/{phase.TotalRepetitions}",
                            FontSize = 11,
                            TextColor = Color.FromArgb("#D8A72D")
                        }
                    }
    ...
    {
        return $"Work {step.WorkSeconds}s · Rest iniziale {step.InitialRestSeconds}s · Rest finale {step.FinalRestSeconds}s · Rip {step.Repetitions}";
    }
    ...
    private static string BuildStepPayloadSummary(WorkoutStepDefinition step)
    {
        return step.StepType switch
        {
            WorkoutStepType.Rest => step.RestPayload is null
                ...
                : $"Blink {step.RestPayload.BlinkColor ?? "#FF0000"} · fine {step.RestPayload.CompletedColor ?? "#00FF00"} · periodo {step.RestPayload.BlinkPeriodMs ?? 500} ms",
            WorkoutStepType.Resistance => step.ResistancePayload is null
                ? "Resistenza libera con parametri di default."
                : $"{step.ResistancePayload.ActiveMode} · attivo {step.ResistancePayload.ActiveColor ?? "#FF8C00"} · fine {step.ResistancePayload.CompletedColor ?? "#00FF00"}",
            WorkoutStepType.Hang => step.HangPayload is null
                ? "Sospensione con target non definiti."
                : $"Prese {string.Join(", ", step.HangPayload.TargetHoleNumbers)} · attivo {step.HangPayload.ActiveColor ?? "#00BFFF"} · fine {step.HangPayload.CompletedColor ?? "#00FF00"}",
            WorkoutStepType.Circuit => step.CircuitPayload is null
                ? "Circuito non definito."
                : $"{step.CircuitPayload.CircuitName} · modalita {step.CircuitPayload.Mode}",
            _ => string.Empty
        };
    }
    ...
    private static string BuildWorkoutStepTypeSummary(WorkoutDefinition workout)
    {
        var labels = workout.Steps
            ...
            .ToList();
        return labels.Count == 0 ? "Nessun tipo step." : string.Join(" · ", labels);
    }
    ...
    private void SetActiveRestColorTarget(RestColorTarget target)
    {
        activeRestColorTarget = target;
        ColorPickerTargetLabel.Text = target switch
        {
            RestColorTarget.Blink => "Stai modificando: recupero rosso lampeggiante",
            RestColorTarget.Completed => "Stai modificando: conferma verde finale",
            _ => "Seleziona un colore da modificare."
        };

        var hex = target switch
        {
            RestColorTarget.Blink => BlinkColorValueLabel.Text ?? "#FF0000",
            RestColorTarget.Completed => CompletedColorValueLabel.Text ?? "#00FF00",
            _ => "#3A3120"
        };
        ...
        isUpdatingRestColorControls = true;
        ...
        isUpdatingRestColorControls = false;
        UpdateRestColorPickerTexts(color);
    }

    private void SetActiveResistanceColorTarget(ResistanceColorTarget target)
    {
        activeResistanceColorTarget = target;
        ResistanceColorPickerTargetLabel.Text = target switch
        {
            ResistanceColorTarget.Active => "Stai modificando: colore attivita resistenza",
            ResistanceColorTarget.Completed => "Stai modificando: conferma verde finale",
            _ => "Seleziona un colore da modificare."
        };

        var hex = target switch
        {
            ResistanceColorTarget.Active => ResistanceActiveColorValueLabel.Text ?? "#FF8C00",
            ResistanceColorTarget.Completed => ResistanceCompletedColorValueLabel.Text ?? "#00FF00",
            _ => "#3A3120"
        };
        ...
        isUpdatingResistanceColorControls = true;
        ResistanceRedSlider.Value = Math.Round(color.Red * 255d);
        ResistanceGreenSlider.Value = Math.Round(color.Green * 255d);
        ResistanceBlueSlider.Value = Math.Round(color.Blue * 255d);
        isUpdatingResistanceColorControls = false;
        UpdateResistanceColorPickerTexts(color);
    }

    private void SetActiveHangColorTarget(HangColorTarget target)
    {
        activeHangColorTarget = target;
        HangColorPickerTargetLabel.Text = target switch
        {
            HangColorTarget.Active => "Stai modificando: colore sospensione",
            HangColorTarget.Completed => "Stai modificando: conferma verde finale",
            _ => "Seleziona un colore da modificare."
        };

        var hex = target switch
        {
            HangColorTarget.Active => HangActiveColorValueLabel.Text ?? "#00BFFF",
            HangColorTarget.Completed => HangCompletedColorValueLabel.Text ?? "#00FF00",
            _ => "#3A3120"
        };
        ...
        isUpdatingHangColorControls = true;
        HangRedSlider.Value = Math.Round(color.Red * 255d);
        HangGreenSlider.Value = Math.Round(color.Green * 255d);
        HangBlueSlider.Value = Math.Round(color.Blue * 255d);
        isUpdatingHangColorControls = false;
        UpdateHangColorPickerTexts(color);
    }

    private void ApplyRestColorToTarget(RestColorTarget target, string hex)
    {
        switch (target)
        {
            case RestColorTarget.Blink:
                BlinkColorValueLabel.Text = hex;
                break;
            case RestColorTarget.Completed:
                CompletedColorValueLabel.Text = hex;
                break;
        }
    ...
    private void ApplyResistanceColorToTarget(ResistanceColorTarget target, string hex)
    {
        switch (target)
        {
            case ResistanceColorTarget.Active:
                ResistanceActiveColorValueLabel.Text = hex;
                break;
            case ResistanceColorTarget.Completed:
    ...
    private void ApplyHangColorToTarget(HangColorTarget target, string hex)
    {
        switch (target)
        {
            case HangColorTarget.Active:
                HangActiveColorValueLabel.Text = hex;
                break;
            case HangColorTarget.Completed:
    ...
    private void UpdateRestColorPickerTexts(Color color)
    {
    ...
    private void UpdateResistanceColorPickerTexts(Color color)
    {
        ResistanceRedValueLabel.Text = $"Rosso: {(int)Math.Round(color.Red * 255d)}";
        ResistanceGreenValueLabel.Text = $"Verde: {(int)Math.Round(color.Green * 255d)}";
        ResistanceBlueValueLabel.Text = $"Blu: {(int)Math.Round(color.Blue * 255d)}";
        ResistanceColorPickerPreview.Color = color;
    }

    private void UpdateHangColorPickerTexts(Color color)
    {
        HangRedValueLabel.Text = $"Rosso: {(int)Math.Round(color.Red * 255d)}";
        HangGreenValueLabel.Text = $"Verde: {(int)Math.Round(color.Green * 255d)}";
        HangBlueValueLabel.Text = $"Blu: {(int)Math.Round(color.Blue * 255d)}";
        HangColorPickerPreview.Color = color;
    }
